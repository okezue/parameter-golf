...
class Hyperparameters:
    ...
    tsqk_enabled = bool(int(os.environ.get("TSQK_ENABLED", "1")))
    tsqk_deep_n = int(os.environ.get("TSQK_DEEP_N", 4))
    tsqk_K = int(os.environ.get("TSQK_K", 64))
    tsqk_G = int(os.environ.get("TSQK_G", 32))
    tsqk_ngains = int(os.environ.get("TSQK_NGAINS", 16))
    tsqk_sketch_dim = int(os.environ.get("TSQK_SKETCH_DIM", 8))
    tsqk_kmeans_iters = int(os.environ.get("TSQK_KMEANS_ITERS", 40))
    tsqk_late_qat_weight = float(os.environ.get("TSQK_LATE_QAT_WEIGHT", 0.15))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if hasattr(self, '_cq') and self._cq is not None:
            q = self._cq(x).reshape(bsz, seqlen, self.num_heads, self.head_dim)
        else:
            q = F.linear(x, q_w.to(x.dtype)).reshape(bsz, seqlen, self.num_heads, self.head_dim)
        if hasattr(self, '_ck') and self._ck is not None:
            k = self._ck(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
        else:
...
# --- TurboSerif-QK: Geometry-Preserving Q/K Codec ---

def _make_gain_table(n=16):
    return torch.logspace(math.log10(2**-3), 0.0, n, dtype=torch.float32)

def _make_H(sketch_dim, G):
    H = torch.randn(sketch_dim, G)
    H = H / H.norm(dim=1, keepdim=True)
    return H

def _activation_weighted_skmeans(groups_np, act_weights_np, K=64, iters=40):
    N, G = groups_np.shape
    norms = np.linalg.norm(groups_np, axis=1, keepdims=True) + 1e-12
    dirs = (groups_np / norms).astype(np.float64)
    w = act_weights_np.astype(np.float64)
    w = w / (w.sum() + 1e-12)
    ...
    for _ in range(iters):
        ...
        for k in range(K):
            ...
            if mask.sum() > 0:
                wm = w[mask, None]
                m = (dirs[mask] * wm).sum(axis=0) / (wm.sum() + 1e-12)
                n = np.linalg.norm(m)
                C[k] = m / n if n > 1e-8 else C[k]
    return C.astype(np.float32)

def tsqk_encode(W_np, cb_np, gain_table_np, H_np, act_groups_np=None):
    R, D = W_np.shape
    K, G = cb_np.shape
    ng = D // G
    nG = len(gain_table_np)
    sD = H_np.shape[0]
    Wg = W_np.reshape(R, ng, G).astype(np.float64)
    mags = np.linalg.norm(Wg, axis=2)
    row_exp = np.ceil(np.log2(np.maximum(mags.max(axis=1), 1e-20))).astype(np.int8)
    dirs = Wg / (mags[:, :, None] + 1e-12)
    df = dirs.reshape(-1, G)
    sims = df @ cb_np.astype(np.float64).T
    bp = sims.max(axis=1)
    bn = (-sims).max(axis=1)
    use_neg = bn > bp
    zi = np.where(use_neg, (-sims).argmax(axis=1), sims.argmax(axis=1)).reshape(R, ng).astype(np.uint8)
    sg = np.where(use_neg, -1.0, 1.0).reshape(R, ng)
    scaled_mags = sg * mags
    target_gains = scaled_mags / (np.power(2.0, row_exp[:, None].astype(np.float64)) + 1e-20)
    gt = gain_table_np.astype(np.float64)
    diffs = np.abs(target_gains[:, :, None] - gt[None, None, :])
    ai = diffs.argmin(axis=2).astype(np.uint8)
    recon_scale = np.power(2.0, row_exp[:, None].astype(np.float64)) * gt[ai]
    cb_vecs = cb_np[zi.ravel()].reshape(R, ng, G).astype(np.float64)
    recon = recon_scale[:, :, None] * cb_vecs
    residual = Wg - recon
    H64 = H_np.astype(np.float64)
    proj_res = np.einsum('hg,rng->rnh', H64, residual)
    sketch = np.sign(proj_res).astype(np.int8)
    sketch[sketch == 0] = 1
    return {
        'zi': zi, 'ai': ai, 're': row_exp, 'sk': sketch,
        'cb': cb_np.astype(np.float16),
        'shape': W_np.shape,
    }

def tsqk_fit_beta(W_np, enc, gain_table_np, H_np, act_groups_np=None):
    R, D = W_np.shape
    K, G = enc['cb'].shape
    ng = D // G
    Wg = W_np.reshape(R, ng, G).astype(np.float64)
    cb = enc['cb'].astype(np.float64)
    gt = gain_table_np.astype(np.float64)
    H64 = H_np.astype(np.float64)
    recon_scale = np.power(2.0, enc['re'][:, None].astype(np.float64)) * gt[enc['ai']]
    cb_vecs = cb[enc['zi'].ravel()].reshape(R, ng, G)
    coarse = recon_scale[:, :, None] * cb_vecs
    residual = Wg - coarse
    proj_res = np.einsum('hg,rng->rnh', H64, residual)
    sketch = enc['sk'].astype(np.float64)
    Ht_b = np.einsum('hg,rnh->rng', H64, sketch)
    num = (residual * Ht_b).sum()
    den = (Ht_b * Ht_b).sum() + 1e-12
    return float(num / den)

class CompressedLinearQK(nn.Module):
    def __init__(self, codebook, row_exp, code_idx, gain_idx, sketch, beta, gain_table, H):
        super().__init__()
        self.register_buffer('cb', codebook)
        self.register_buffer('re', row_exp.float())
        self.register_buffer('zi', code_idx.long())
        self.register_buffer('ai', gain_idx.long())
        self.register_buffer('sk', sketch.float())
        self.register_buffer('gt', gain_table)
        self.register_buffer('H', H)
        self.beta = nn.Parameter(torch.tensor(beta, dtype=torch.float32))
        R, ng = code_idx.shape
        G = codebook.shape[1]
        scales = (2.0 ** row_exp.float()).unsqueeze(-1) * gain_table[gain_idx.long()]
...
def _ts_decode_full(enc, gt_np, H_np, beta):
    ...
    G = enc['cb'].shape[1]
    cb = enc['cb'].astype(np.float64)
    gt = gt_np.astype(np.float64)
    H64 = H_np.astype(np.float64)
    scale = np.power(2.0, enc['re'][:, None].astype(np.float64)) * gt[enc['ai']]
    vecs = cb[enc['zi'].ravel()].reshape(R, ng, G)
    coarse = scale[:, :, None] * vecs
    sketch = enc['sk'].astype(np.float64)
    Ht_b = np.einsum('hg,rnh->rng', H64, sketch)
    W = (coarse + beta * Ht_b).reshape(R, ng * G)
    return W.astype(np.float32)

def tsqk_build_compressed_linear(enc, beta, gain_table, H, device):
    cb = torch.from_numpy(enc['cb'].astype(np.float32)).to(device)
    re = torch.from_numpy(enc['re'].astype(np.float32)).to(device)
    zi = torch.from_numpy(enc['zi'].astype(np.int64)).to(device)
    ai = torch.from_numpy(enc['ai'].astype(np.int64)).to(device)
    sk = torch.from_numpy(enc['sk'].astype(np.float32)).to(device)
    return CompressedLinearQK(cb, re, zi, ai, sk, beta, gain_table.to(device), H.to(device))

def tsqk_pack(enc, beta):
    pi = ((enc['zi'].ravel().astype(np.uint16) << 4) | enc['ai'].ravel().astype(np.uint16)).astype(np.uint16)
    sk_packed = np.packbits((enc['sk'].ravel() > 0).astype(np.uint8))
    return {
        'pi': torch.from_numpy(pi),
        'sk': torch.from_numpy(sk_packed),
...
def tsqk_unpack(pk, G=32, sD=8, nG=16):
    pi = pk['pi'].numpy().astype(np.uint16)
    zi = (pi >> 4).astype(np.uint8)
    ai = (pi & 0x0F).astype(np.uint8)
    R = pk['re'].shape[0]
    ng = pk['shape'][1] // G
    zi = zi.reshape(R, ng)
    ai = ai.reshape(R, ng)
    sk_bits = np.unpackbits(pk['sk'].numpy())[:R*ng*sD].reshape(R, ng, sD)
    sk = (sk_bits.astype(np.int8) * 2 - 1)
    return {
        'zi': zi, 'ai': ai, 're': pk['re'].numpy(),
        'sk': sk, 'cb': pk['cb'].numpy(),
        'shape': pk['shape'],
    }, float(pk['beta'].item())

def tsqk_nbytes(pk):
    return pk['pi'].numel()*2 + pk['sk'].numel() + pk['re'].numel() + pk['cb'].numel()*2 + 4
...
def main() -> None:
    ...
    tsqk_data = {}
    tsqk_names = set()
    if args.tsqk_enabled:
        deep_start = args.num_layers - args.tsqk_deep_n
        gt_np = _make_gain_table(args.tsqk_ngains).numpy()
        H_t = _make_H(args.tsqk_sketch_dim, args.tsqk_G)
        H_np = H_t.numpy()
        all_groups = []
        all_weights_for_cb = []
        qk_targets = []
        for i in range(deep_start, args.num_layers):
            for suffix in ['c_q.weight', 'c_k.weight']:
                name = f'blocks.{i}.attn.{suffix}'
                if name in unbanked_sd:
                    qk_targets.append(name)
                    W = unbanked_sd[name].float().numpy()
                    R, D = W.shape
                    Wg = W.reshape(R, D // args.tsqk_G, args.tsqk_G)
                    norms = np.linalg.norm(Wg, axis=2).ravel()
                    dirs = (Wg / (norms.reshape(-1, 1, 1)[:Wg.shape[0]*Wg.shape[1]].reshape(Wg.shape[0], Wg.shape[1], 1) + 1e-12))
                    all_groups.append(dirs.reshape(-1, args.tsqk_G))
                    all_weights_for_cb.append(norms.reshape(-1))
        if qk_targets:
            all_g = np.concatenate(all_groups, axis=0)
            all_w = np.concatenate(all_weights_for_cb, axis=0)
            log0(f"tsqk:fitting codebook K={args.tsqk_K} on {len(all_g)} groups")
            cb = _activation_weighted_skmeans(all_g, all_w, K=args.tsqk_K, iters=args.tsqk_kmeans_iters)
            for name in qk_targets:
                tsqk_names.add(name)
                W = unbanked_sd[name].float().numpy()
                enc = tsqk_encode(W, cb, gt_np, H_np)
                beta = tsqk_fit_beta(W, enc, gt_np, H_np)
                pk = tsqk_pack(enc, beta)
                tsqk_data[name] = {'enc': enc, 'beta': beta, 'pk': pk}
                Wr = _ts_decode_full(enc, gt_np, H_np, beta)
                mse = float(np.mean((W - Wr) ** 2))
                cos = float(np.sum(W * Wr) / (np.linalg.norm(W) * np.linalg.norm(Wr) + 1e-8))
                log0(f"  {name}: mse={mse:.6f} cos={cos:.6f} bytes={tsqk_nbytes(pk)}")
    quant_sd_for_int6 = {k: v for k, v in unbanked_sd.items() if k not in tsqk_names}
    quant_result, quant_meta = mixed_quantize_int6(quant_sd_for_int6, {"mlp", "attn"})
    for name, td in tsqk_data.items():
        pk = td['pk']
        for sfx in ['pi', 'sk', 're', 'cb', 'beta']:
            quant_result[f"{name}.tsqk_{sfx}"] = pk[sfx]
        quant_meta[name] = {"type": "tsqk", "shape": pk['shape']}
    ...
    if args.tsqk_enabled and tsqk_data:
        gt_t = _make_gain_table(args.tsqk_ngains).to(device)
        H_t2 = _make_H(args.tsqk_sketch_dim, args.tsqk_G).to(device)
        deep_start = args.num_layers - args.tsqk_deep_n
        for name, td in tsqk_data.items():
            parts = name.split('.')
            bi = int(parts[1])
            is_q = 'c_q' in name
            enc_unpacked, beta = tsqk_unpack(td['pk'], G=args.tsqk_G, sD=args.tsqk_sketch_dim, nG=args.tsqk_ngains)
            cmod = tsqk_build_compressed_linear(enc_unpacked, beta, gt_t, H_t2, device)
            if is_q:
                eval_model.blocks[bi].attn._cq = cmod
            else:
                eval_model.blocks[bi].attn._ck = cmod
            log0(f"tsqk:attached compressed {name} to block {bi}")
