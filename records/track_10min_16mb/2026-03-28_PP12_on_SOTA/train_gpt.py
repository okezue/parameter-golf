...
class PacketBuilder:
    ...
    def update(self, toks):
        ...
        for i in range(1, len(toks)):
            self.bi[int(toks[i-1]), int(toks[i])] += 1.0
            self.bi_tot[int(toks[i-1])] += 1.0
    def build(self):
        prior = np.zeros((self.V, self.V), dtype=np.float32)
        tau = np.zeros(self.V, dtype=np.float32)
        for p in range(self.V):
            if self.bi_tot[p] > 0:
                prior[p] = (self.bi[p] / self.bi_tot[p]).astype(np.float32)
                tau[p] = min(self.bi_tot[p] / 5.0, 100.0)
        return torch.from_numpy(prior), torch.from_numpy(tau)

class PacketStore:
    def __init__(self, V, prior, tau, device):
        self.V = V
        self.prior = prior.to(device)
        self.tau = tau.to(device)
        self.online = torch.zeros(V, V, device=device, dtype=torch.float32)
        self.online_tot = torch.zeros(V, device=device, dtype=torch.float32)
    @torch.no_grad()
    def update_online(self, tokens):
        ...
        prev = tokens[:-1].long()
        curr = tokens[1:].long()
        ones = torch.ones(prev.numel(), device=self.prior.device, dtype=torch.float32)
        self.online.view(-1).scatter_add_(0, prev * self.V + curr, ones)
        self.online_tot.scatter_add_(0, prev, ones)
    @torch.no_grad()
    def predict(self, prev_ids):
        t = self.tau[prev_ids].unsqueeze(-1).clamp_min(0.1)
        p = self.prior[prev_ids]
        o = self.online[prev_ids]
        ot = self.online_tot[prev_ids].unsqueeze(-1)
        post = (o + t * p) / (ot + t).clamp_min(1e-8)
        return post / post.sum(-1, keepdim=True).clamp_min(1e-8)

def eval_val_packet(args, base_model, rank, world_size, device, val_tokens,
                    base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                    pstore, stride, batch_seqs=32, log0=print):
    ...
    for ci in range(num_chunks):
        ...
        with torch.inference_mode():
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                for i, ws in enumerate(bws):
                    ...
                    has_data = ((pstore.tau[prev] + pstore.online_tot[prev]) > 20.0).float()
        ...
        pstore.update_online(val_tokens[cs:ce+1].to(device=device, dtype=torch.int64))
        if rank == 0 and (ci % 100 == 0 or ci == num_chunks - 1):
            ...
            log0(f"  pkt [{ci+1}/{num_chunks}] bpb={rb:.6f} time={el:.1f}s")
    if dist.is_available() and dist.is_initialized():
    ...
    base_model.train()
    log0(f"packet_eval:done val_loss={vl:.6f} val_bpb={vb:.6f} elapsed={time.perf_counter()-t0:.1f}s")
    return vl, vb
...
def main() -> None:
    ...
    log0("pp:building packets from training data...")
    ...
    pstore = PacketStore(args.vocab_size, pp_prior, pp_tau, device)
    log0(f"pp:done {pp_seen/1e6:.0f}M tokens, active={int((pp_tau>0).sum())}, time={time.perf_counter()-t_pp:.1f}s")
    del pb, pp_stream
    ...
    pstore.online.zero_()
    pstore.online_tot.zero_()
    torch.cuda.synchronize()
    t_pkt = time.perf_counter()
    pkt_loss, pkt_bpb = eval_val_packet(
        args, eval_model, rank, world_size, device,
        val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        pstore, stride=args.eval_stride, log0=log0)
    torch.cuda.synchronize()
