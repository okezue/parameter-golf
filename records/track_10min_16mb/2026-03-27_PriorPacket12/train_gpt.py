...
class Hyperparameters:
    ...
    pp_max_order = int(os.environ.get("PP_MAX_ORDER", 12))
    pp_dense_buckets = int(os.environ.get("PP_DENSE_BUCKETS", 65536))
    pp_sparse_topk = int(os.environ.get("PP_SPARSE_TOPK", 100000))
    pp_top_conts = int(os.environ.get("PP_TOP_CONTS", 4))
    pp_gate_hidden = int(os.environ.get("PP_GATE_HIDDEN", 12))
...
# --- PriorPacket: Posterior Packet Builder & Store ---

_FNV_OFF = np.uint64(14695981039346656037)
_FNV_MUL = np.uint64(1099511628211)

def _fnv(toks, mask):
    h = _FNV_OFF
    for t in toks:
        h = (h ^ np.uint64(t)) * _FNV_MUL
...
class PriorPacketStore:
    ...
    @torch.no_grad()
    def update_online(self, tokens):
        if tokens.numel() < 2:
            return
        prev = tokens[:-1].long()
        curr = tokens[1:].long()
        idx = prev * self.V + curr
        ones = torch.ones(idx.numel(), device=self.device, dtype=torch.float32)
...
class ExpertGate(nn.Module):
    def __init__(self, n_features=9, hidden=12, n_experts=2):
        super().__init__()
    ...
    def forward(self, features):
        return F.softmax(self.fc2(F.relu(self.fc1(features))), dim=-1)
...
class GPT(nn.Module):
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads, mlp_mult,
                 ...
                 bigram_vocab_size=0, bigram_dim=64, xsa_last_n=0, rope_dims=0, ln_scale=False):
        super().__init__()
        ...
        if rope_dims > 0:
            ...
            for block in self.blocks:
                ...
                block.attn.rotary = Rotary(hd, base=rope_base, rope_dims=rope_dims)
        if xsa_last_n > 0:
...
# --- PriorPacket eval: Bayesian posterior + neural mixing ---

def eval_val_packet(args, base_model, rank, world_size, device, val_tokens,
                    base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                    packet_store, stride, batch_seqs=32, log0=print):
    ...
    for ci in range(num_chunks):
        ...
        with torch.inference_mode():
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                p_nn = F.softmax(logits.float(), dim=-1)
                for i, ws in enumerate(batch_ws):
                    wlen = wlens[i]
        ...
        packet_store.update_online(val_tokens[cs:ce+1].to(device=device, dtype=torch.int64))
        if rank == 0 and (ci % 50 == 0 or ci == num_chunks - 1):
            ...
            log0(f"  packet [{ci+1}/{num_chunks}] bpb={rb:.6f} time={el:.1f}s")
    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(loss_sum, op=dist.ReduceOp.SUM)
        dist.all_reduce(token_count, op=dist.ReduceOp.SUM)
        dist.all_reduce(byte_count, op=dist.ReduceOp.SUM)
    ...
    val_bpb = val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    base_model.train()
    log0(f"packet_eval:done val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} elapsed={time.perf_counter()-t0:.1f}s")
    return val_loss, val_bpb
...
def main():
    ...
    optimizer_tok = torch.optim.Adam(tok_params, betas=(args.beta1, args.beta2), eps=args.adam_eps, fused=True)
    ...
    # --- Complementary training: build bigram surrogate for loss weighting ---
    comp_bi = None
    if args.comp_train:
        comp_bi = torch.zeros(args.vocab_size, args.vocab_size, device=device, dtype=torch.float32)
        comp_tot = torch.zeros(args.vocab_size, device=device, dtype=torch.float32)
        log0("comp_train:enabled (dataloader-side loss weighting)")
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            if comp_bi is not None:
                with torch.no_grad():
                    prev = x.reshape(-1).long()
                    curr = y.reshape(-1).long()
                    idx = prev * args.vocab_size + curr
                    ones = torch.ones(idx.numel(), device=device, dtype=torch.float32)
                    comp_bi.view(-1).scatter_add_(0, idx, ones)
                    comp_tot.scatter_add_(0, prev, ones)
    ...
    torch.cuda.synchronize()
    diag_val_loss, diag_val_bpb = eval_val(args, compiled_model if not distributed else model, rank, world_size, device,
                                            grad_accum_steps, val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut)
    ...
    if master_process:
        ...
        with open("packets.ptz", "wb") as f:
            f.write(packet_blob)
        packet_bytes = len(packet_blob)
        total = quant_file_bytes + code_bytes + packet_bytes
        log0(f"Packet store: {packet_bytes} bytes")
    ...
    pkt_loss, pkt_bpb = eval_val_packet(
        args, base_model, rank, world_size, device,
        val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        packet_store, stride=args.eval_stride, log0=log0)
    torch.cuda.synchronize()
    log0(f"packet_eval val_loss:{pkt_loss:.4f} val_bpb:{pkt_bpb:.4f} "
         f"eval_time:{1000.0 * (time.perf_counter() - t_pkt):.0f}ms")
    log0(f"packet_eval_exact val_loss:{pkt_loss:.8f} val_bpb:{pkt_bpb:.8f}")
    log0(f"final_int8_zlib_roundtrip_exact val_loss:{pkt_loss:.8f} val_bpb:{pkt_bpb:.8f}")
    if distributed:
